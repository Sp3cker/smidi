...
export async function readMidiFile(
  path: string,
  opts: ReaderOptions = DefaultReaderOptions
): Promise<SongModel> {
  ...
  const division: Division =
    typeof parsed.header.ticksPerBeat === "number"
      ? { type: "tpq", ticksPerQuarter: parsed.header.ticksPerBeat }
      : {
          type: "smpte",
          smpte: parsed.header.framesPerSecond ?? 24,
          ticksPerFrame: parsed.header.ticksPerFrame ?? 80,
        };
...
function convertEvent(ev: ParsedEvent, opts: ReaderOptions): AnyEvent {
  ...
  if (ev.type === "meta") {
    ...
    switch (ev.subtype) {
      case "marker":
        return { delta, type: "marker", text: ev.text } as AnyEvent;
      case "setTempo":
        return {
          delta,
          type: "setTempo",
          microsecondsPerQuarter: ev.microsecondsPerBeat,
        } as AnyEvent;
      case "timeSignature":
        return {
          delta,
          ...
          metronome: ev.metronome,
          thirtyseconds: ev.thirtyseconds,
        } as AnyEvent;
      case "keySignature":
        return {
          delta,
      ...
      case "endOfTrack":
        return { delta, type: "endOfTrack" } as AnyEvent;
      case "text":
        return { delta, type: "text", text: ev.text } as AnyEvent;
      case "trackName":
        return { delta, type: "trackName", text: ev.text } as AnyEvent;
      case "instrumentName":
        return { delta, type: "instrumentName", text: ev.text } as AnyEvent;
      case "cuePoint":
        return { delta, type: "cuePoint", text: ev.text } as AnyEvent;
      case "channelPrefix":
        return {
          delta,
          type: "channelPrefix",
      ...
      default: {
        ...
        return {
          delta,
          type: "unknown",
          metaTypeByte: typeof mtb === "number" ? mtb : 0,
  ...
  if (ev.type === "sysEx" || ev.type === "endSysEx") {
    ...
    return {
      delta,
      type: ev.type as "sysEx" | "endSysEx",
      data: new Uint8Array(d),
    } as AnyEvent;
  }
  // Channel events
  ...
  switch (ev.type) {
    case "noteOn":
      return {
        delta,
        type: "noteOn",
        channel: ((ev as any).channel ?? 0) + 1,
        note: (ev as any).noteNumber ?? 0,
        velocity: (ev as any).velocity ?? 0,
      } as AnyEvent;
    case "noteOff":
      return {
        delta,
    ...
    case "programChange":
      return {
        ...
        type: "programChange",
        channel: ((ev as any).channel ?? 0) + 1,
        program: (ev as any).programNumber ?? 0,
      } as AnyEvent;
    case "controller":
      return {
        delta,
        type: "controller",
        channel: ((ev as any).channel ?? 0) + 1,
        controller: (ev as any).controllerType ?? 0,
        value: (ev as any).value ?? 0,
      } as AnyEvent;
